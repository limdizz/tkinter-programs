...
label_temp = Label(frame_entry, text='\N{DEGREE CELSIUS}', font=50, width=5)
button_convert = Button(root, text='\N{RIGHTWARDS BLACK ARROW}',
                        command=celsius_to_fahrenheit, font=50)
...
def fahrenheit_to_celsius():
    ...
    label_result2["text"] = f"{round(celsius, 2)} \N{DEGREE CELSIUS}"


frame_entry2 = Frame(root)
entry_temperature2 = Entry(frame_entry2, width=20)
label_temp2 = Label(frame_entry2, text='\N{DEGREE FAHRENHEIT}', font=50, width=5)
button_convert2 = Button(root, text='\N{RIGHTWARDS BLACK ARROW}',
                         font=50, command=fahrenheit_to_celsius)
...
frame_entry2.grid(row=1, column=0, padx=10, pady=10)
button_convert2.grid(row=1, column=1, pady=10)
label_result2.grid(row=1, column=2, padx=10)
